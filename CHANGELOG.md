...
### Added
- shadcn-svelte preset `b2W5XQjlI` with "mira" style and olive base color
- Inter variable font (`@fontsource-variable/inter`)
- `shadcn-svelte` runtime package for theme CSS
- `<meta name="color-scheme" content="dark">` to prevent browser dark mode extensions from interfering

### Changed
- Updated `components.json` with preset config (mira style, olive base, bold menu accent)
- Updated `layout.css` with preset theme tokens
- Updated button component to mira style variant
- Updated `src/lib/utils.ts` with `WithoutChild`, `WithoutChildren`, `WithoutChildrenOrChild` types
